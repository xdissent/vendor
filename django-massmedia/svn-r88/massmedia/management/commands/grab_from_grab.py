     ...
     def handle(self, *a, **kw):
          ...
          for element in parser.close().iter("*"):
               if element.tag == 'video':
                    ...
                    title = '%(source-name)s: %(title)s' % d     
                    time = datetime.strptime(d['created-at'],'%Y-%m-%dT%H:%M:%SZ')
                    slug,s = slugify(title)[:50],slugify(time.strftime("%d-%b-%Y %H:%M:%S").lower() + title)[:50]
                    try:
                         video = GrabVideo.objects.get(slug=slug)
                         continue
                    except GrabVideo.DoesNotExist:
                         thumb = Image.objects.create(slug=s)
                         ...
                         if not ',' in unicode(d['keywords'] or ''):
                              ...
                              d['keywords'] = map(lambda x: unicode(x.strip())[:50], unicode(d['keywords'] or '').split(','))
                         video = GrabVideo.objects.create(
                              title = title,
                              slug = slug,
                              creation_date = time,
